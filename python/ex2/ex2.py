...
def plot_boundary(theta, X, y):
    ...
    # 两个端点确定直线
    plot_x = np.array([np.min(X[:, 1], axis=0) - 2, np.max(X[:, 1], axis=0)])
    plot_y = np.array(
        (-1.0 / theta[2]) * (theta[1] * plot_x + theta[0]))
...
if __name__ == '__main__':
    ...
    # part4 预测并计算准确率
    print('测试的45 85分，估计值应该在0.775 +/- 0.002')
    tmp_x = np.array([1, 45, 85])
    print(sigmoid(np.dot(tmp_x, theta)))

    # 预测准确率,
    p = predict(theta, X)
    print('准确率为{}%'.format(np.mean(p == y) * 100))
