...
if __name__ == '__main__':
    ...
    # part1 将数据进行映射，进行logistic回归
    X = mapFeature(X[:, 0], X[:, 1])
    lambda_ = 1
    initial_theta = np.zeros((X.shape[1], 1))
    J = cost_function(initial_theta, X, y, lambda_)
    print('预期的损失应该在0.693左右')
    print(J)
    grad = grad_function(initial_theta, X, y, lambda_)
    print('预期的grad前几位为0.0085 0.0188 0.0001 0.0503 0.0115')
    print(grad.flatten()[:5])
    ...
    print('预期的grad前几位为0.3460 0.1614 0.1948 0.2269 0.0922')
    print(grad.flatten()[:5])
